...
def add_regime(month_series: pd.Series) -> pd.Series:
    return np.select(
        [
            month_series.isin([5, 9]),
            month_series.isin([6, 7, 8]),
        ],
        ["transition", "peak_summer"],
        default="regular",
    )
...
def fit_best_ridge_model(processed_splits: dict[str, pd.DataFrame], columns: list[str]) -> tuple[float, Pipeline]:
    ...
    for alpha in ALPHA_GRID:
        ...
        valid_pred = enforce_operating_hours(processed_splits["valid"], non_negative_only(model.predict(x_valid)))
...
def fit_weather_only_model(df: pd.DataFrame) -> tuple[dict[str, pd.DataFrame], float]:
    ...
    alpha, model = fit_best_ridge_model(processed, columns)

    for split_name in ["train", "valid", "test"]:
        preds = non_negative_only(model.predict(processed[split_name][columns]))
        preds = enforce_operating_hours(processed[split_name], preds)
        processed[split_name]["base_prediction"] = preds
    return processed, alpha


def fit_linear_calibrator(x: np.ndarray, y: np.ndarray) -> LinearRegression:
    model = LinearRegression()
    model.fit(x.reshape(-1, 1), y)
    return model


def apply_train_based_calibration(processed: dict[str, pd.DataFrame]) -> tuple[dict[str, pd.DataFrame], pd.DataFrame]:
    train_frame = processed["train"]
    global_calibrator = fit_linear_calibrator(
        train_frame["base_prediction"].to_numpy(dtype=float),
        train_frame[TARGET].to_numpy(dtype=float),
    )

    regime_calibrators = {}
    for regime in ["transition", "peak_summer", "regular"]:
        sub = train_frame[train_frame["regime"] == regime].copy()
        regime_calibrators[regime] = fit_linear_calibrator(
            sub["base_prediction"].to_numpy(dtype=float),
    ...
    coef_rows = [
        {
            ...
            "regime": "all",
            "coef": float(global_calibrator.coef_[0]),
            "intercept": float(global_calibrator.intercept_),
        }
    ]
    for regime, calibrator in regime_calibrators.items():
        coef_rows.append(
            {
                "calibration_type": "by_regime",
                "regime": regime,
    ...
    for split_name in ["train", "valid", "test"]:
        frame = processed[split_name]
        global_pred = global_calibrator.predict(frame["base_prediction"].to_numpy(dtype=float).reshape(-1, 1))
        frame["global_calibrated_prediction"] = enforce_operating_hours(frame, global_pred)

        regime_pred = np.empty(len(frame), dtype=float)
        for regime, calibrator in regime_calibrators.items():
            mask = frame["regime"] == regime
            if mask.any():
                regime_pred[mask.to_numpy()] = calibrator.predict(
                    frame.loc[mask, "base_prediction"].to_numpy(dtype=float).reshape(-1, 1)
                )
        frame["regime_calibrated_prediction"] = enforce_operating_hours(frame, regime_pred)

    return processed, pd.DataFrame(coef_rows)


def build_metrics_df(processed: dict[str, pd.DataFrame]) -> pd.DataFrame:
    rows = []
    model_map = [
        ("weather_only_extended", "base_prediction"),
        ("weather_only_extended_global_calibrated", "global_calibrated_prediction"),
        ("weather_only_extended_regime_calibrated", "regime_calibrated_prediction"),
    ]
    for split_name in ["train", "valid", "test"]:
        frame = processed[split_name]
        for model_name, pred_col in model_map:
            rows.append({"model_name": model_name, "scope": split_name, **calc_metrics(frame[TARGET], frame[pred_col])})
                    ...
                    rows.append(
                        {
                            ...
                            "scope": f"test_{regime}",
                            "regime": regime,
                            **calc_metrics(sub[TARGET], sub[pred_col]),
                        }
                    )
    return pd.DataFrame(rows)


def build_monthly_df(processed: dict[str, pd.DataFrame]) -> pd.DataFrame:
    full_df = pd.concat([processed["train"], processed["valid"], processed["test"]], ignore_index=True).sort_values("datetime")
    return (
        full_df.assign(month_start=full_df["datetime"].dt.to_period("M").dt.to_timestamp())
        .groupby("month_start", as_index=False)[
            [TARGET, "base_prediction", "global_calibrated_prediction", "regime_calibrated_prediction"]
        ]
        .mean()
    )


def render_graphs(monthly_df: pd.DataFrame, metrics_df: pd.DataFrame, output_path: Path) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)

    fig, axes = plt.subplots(2, 1, figsize=(16, 11), height_ratios=[1.7, 1.0])

    ax = axes[0]
    ax.plot(monthly_df["month_start"], monthly_df[TARGET], label="actual", linewidth=2.5, color="#1f4e79")
    ax.plot(monthly_df["month_start"], monthly_df["base_prediction"], label="weather_only_extended", linestyle="--", linewidth=2.0, color="#d97a04")
    ax.plot(
        monthly_df["month_start"],
        monthly_df["global_calibrated_prediction"],
        label="global calibrated",
        linestyle="-.",
        linewidth=2.1,
        color="#2e8b57",
    )
    ax.plot(
        monthly_df["month_start"],
        monthly_df["regime_calibrated_prediction"],
        label="regime calibrated",
        linestyle=":",
        linewidth=2.3,
        color="#7a4fb0",
    )
    ax.set_title("Monthly Mean Comparison (2022-2025): Weather Only Calibration")
    ax.set_xlabel("month")
    ...
    bar_ax = axes[1]
    plot_df = metrics_df[metrics_df["scope"].isin(["test", "test_transition", "test_peak_summer", "test_regular"])].copy()
    plot_df["scope_label"] = plot_df["scope"].map(
        {
            "test": "All Test",
            "test_transition": "Transition",
            "test_peak_summer": "Peak Summer",
            "test_regular": "Regular",
        }
    )
    scope_order = ["All Test", "Transition", "Peak Summer", "Regular"]
    model_order = [
        "weather_only_extended",
        "weather_only_extended_global_calibrated",
        "weather_only_extended_regime_calibrated",
    ]
    colors = ["#d97a04", "#2e8b57", "#7a4fb0"]
    x = np.arange(len(scope_order))
    width = 0.24

    for idx, (model_name, color) in enumerate(zip(model_order, colors)):
        values = plot_df[plot_df["model_name"] == model_name].set_index("scope_label").reindex(scope_order)["rmse"].to_numpy()
        bar_ax.bar(x + (idx - 1) * width, values, width=width, color=color, label=model_name.replace("weather_only_extended_", "").replace("weather_only_extended", "base"))

    bar_ax.set_title("Test RMSE Comparison: Base vs Calibration")
    bar_ax.set_xticks(x)
    bar_ax.set_xticklabels(scope_order)
    bar_ax.set_ylabel("RMSE")
    bar_ax.grid(axis="y", alpha=0.2, linestyle="--")
...
def main() -> None:
    df = load_dataset()
    processed, alpha = fit_weather_only_model(df)
    processed, coef_df = apply_train_based_calibration(processed)
    metrics_df = build_metrics_df(processed)
    monthly_df = build_monthly_df(processed)

    graph_path = OUTPUT_DIR / "nanji_weather_only_calibration_experiment.png"
    monthly_path = OUTPUT_DIR / "nanji_weather_only_calibration_monthly_mean.csv"
    metrics_path = OUTPUT_DIR / "nanji_weather_only_calibration_metrics.csv"
    coef_path = OUTPUT_DIR / "nanji_weather_only_calibration_coefficients.csv"

    render_graphs(monthly_df, metrics_df, graph_path)
    ...
    print(f"saved calibration coefficients: {coef_path}")
